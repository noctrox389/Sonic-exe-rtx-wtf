...
function onEvent(name,v1,v2)
    if name == 'Fatality Popup' and PopupCreate == false then
        ...
        if getProperty('blackBorderOldSonic.x') ~= 'blackBorderOldSonic.x' then
            ...
            popX = {-50,screenWidth - 370}
        end
        makeAnimatedLuaSprite('FatalityPopup'..(PopupLength + 1),'error_popups',math.random(popX[1],popX[2]),math.random(0,screenHeight - 264))
...
function onUpdate()
    for PopupCounter = 1,PopupLength do
        if PopupsDeleted[PopupCounter] == false then
            if getMouseX('other') >= getProperty('FatalityPopup'..PopupCounter..'.x') + (getProperty('FatalityPopup'..PopupCounter..'.width')/2.4) and getMouseX('other') <= getProperty('FatalityPopup'..PopupCounter..'.x') + (getProperty('FatalityPopup'..PopupCounter..'.width')/1.75) and  getMouseY('other') >= getProperty('FatalityPopup'..PopupCounter..'.y') + (getProperty('FatalityPopup'..PopupCounter..'.height')/1.5) and getMouseY('other') <= getProperty('FatalityPopup'..PopupCounter..'.y') + (getProperty('FatalityPopup'..PopupCounter..'.height')/1.05)  and getPropertyFromClass('flixel.FlxG','mouse.justPressed') == true then
